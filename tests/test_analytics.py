...
def test_analytics_summary(client, app_ctx):
    """Проверяем структуру ответа эндпоинта /api/analytics/summary."""
    ...
    addr1 = Address(
        name="A1", lat=0, lon=0, status="Локальный доступ", category="Видеонаблюдение"
    )
    addr2 = Address(
        name="A2", lat=0, lon=0, status="Удаленный доступ", category="Домофон"
    )
    addr3 = Address(
        name="A3", lat=0, lon=0, status="Удаленный доступ", category="Видеонаблюдение"
    )
