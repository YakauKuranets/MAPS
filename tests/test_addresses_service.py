...
def test_filter_addresses_basic(db_session):
    a1 = Address(
        name="Shop", lat=1.0, lon=2.0, category="store", status="open", notes="Продукты"
    )
    a2 = Address(
        name="School", lat=3.0, lon=4.0, category="edu", status="closed", notes="Школа"
    )
    a3 = Address(
        name="Big Shop",
        lat=5.0,
...
def test_export_and_import_addresses_roundtrip(app, db_session):
    a1 = Address(
        name="A1", lat=1.0, lon=2.0, category="cat", status="open", notes="one"
    )
    a2 = Address(
        name="A2", lat=3.0, lon=4.0, category="cat", status="closed", notes="two"
    )
