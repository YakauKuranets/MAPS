...
def test_export_addresses_root_uses_addresses_service(monkeypatch, app, db_session):
    ...
    addr = Address(
        name="A1", lat=1.0, lon=2.0, category="cat", status="open", notes="n"
    )
    db.session.add(addr)
    db.session.commit()
    ...
    monkeypatch.setattr(
        "app.services.general_service.export_addresses_csv", fake_export
    )
...
def test_import_addresses_root_uses_addresses_service(monkeypatch, app, db_session):
    csv_data = (
        "id,name,lat,lon,notes,status,link,category\n,New,1.0,2.0,desc,open,,cat\n"
    )
    ...
    monkeypatch.setattr(
        "app.services.general_service.import_addresses_from_csv", fake_import
    )
