...
class TestScenarioGenerator:
    ...
    def __init__(self, llm_endpoint: Optional[str] = None):
        self.llm = (
            PredictiveAdvisor(endpoint=llm_endpoint)
            if llm_endpoint
            else PredictiveAdvisor()
        )
    ...
    def fetch_cve_details(self, cve_id: str) -> Dict:
        ...
        try:
            response = requests.get(
                self.NVD_API_BASE, params={"cveId": cve_id}, timeout=10
            )
    ...
    def generate_test_script(self, cve_details: Dict) -> str:
        ...
        prompt = f"""
Based on the following CVE details, create a safe proof-of-concept script
in Python or Bash to verify the vulnerability.
The script should be educational, non-destructive, and include comments
...
""".strip()
    ...
    def generate(self, cve_id: str) -> Dict:
        ...
        if not details:
            return {
                "cve_id": cve_id,
                "error": "CVE not found or no details available",
                "script": None,
            }
