...
def audit_http_headers(url: str, timeout: float = 5.0) -> HeaderAuditResult:
    ...
    try:
        ...
        # Score: 100 - penalties
        penalties = (
            len(result.leaked_headers) * 15 + len(result.missing_security_headers) * 10
        )
...
def run_stealth_audit(target: str, ports: list[int]) -> StealthReport:
    ...
    for port in ports:
        ...
        if result.state == "open":
            logger.info(
                "  Порт %d: OPEN (%.1fms) banner=%s",
                port,
                result.response_time_ms,
                result.service_banner or "—",
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--ports", default="80,443,8000,8080,3000,5000", help="Comma-separated ports"
    )
    parser.add_argument(
        "--output", default="/tmp/stealth_report.json", help="Output JSON path"
    )
    ...
    logger.info(
        "═══ ВЕРДИКТ: %s (Score: %.1f/100) ═══", report.verdict, report.overall_score
    )
