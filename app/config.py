...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get(
        "DATABASE_URI",
        settings.database_uri or f"sqlite:///{os.path.join(BASE_DIR, 'app.db')}",
    )
    ...
    # Время жизни "постоянной" сессии
    # (используется после login, когда session.permanent=True)
    PERMANENT_SESSION_LIFETIME = timedelta(
        hours=int(os.environ.get("SESSION_LIFETIME_HOURS", 12))
    )

    # JWT настройки
    JWT_SECRET_KEY = os.environ.get("JWT_SECRET_KEY") or _safe_secret_key()
    ...
    LEAK_CLASSIFIER_PATH = os.environ.get(
        "LEAK_CLASSIFIER_PATH", "/models/leak-classifier"
    )
    ...
    HANDSHAKE_UPLOAD_FOLDER = os.environ.get(
        "HANDSHAKE_UPLOAD_FOLDER", "/data/handshakes"
    )
    HASHCAT_WORDLIST = os.environ.get(
        "HASHCAT_WORDLIST", "/data/wordlists/rockyou_optimized.txt"
    )

    # --- AI feedback optimization (diagnostics quality improvement) ---
    AI_FEEDBACK_DATASET_PATH = os.environ.get(
        "AI_FEEDBACK_DATASET_PATH", "/data/ai/diagnostics_feedback.jsonl"
    ).strip()
    AI_FINETUNE_SCRIPT = os.environ.get(
        "AI_FINETUNE_SCRIPT", "/opt/ai/finetune_llm.py"
    ).strip()
    HANDSHAKE_MAX_FILE_SIZE_BYTES = int(
        os.environ.get("HANDSHAKE_MAX_FILE_SIZE_BYTES", str(10 * 1024 * 1024))
    )
    ...
    # Порт отдельного WS-сервера (dev/legacy режим).
    # В проде рекомендуем ASGI-вариант (/ws на том же порту).
    WS_PORT = int(os.environ.get("WS_PORT", 8765))
    REALTIME_DISABLE_SAMEPORT = os.environ.get(
        "REALTIME_DISABLE_SAMEPORT", "1"
    )  # 1=не пытаться /ws на том же порту Flask
    # TTL токена (секунды) для подключения к realtime.
    REALTIME_TOKEN_TTL_SEC = int(os.environ.get("REALTIME_TOKEN_TTL_SEC", 600))
    # Разрешённые Origin'ы для WS (через запятую).
    # Если пусто — разрешаем same-site и Origin=None (CLI).
    ...
    REALTIME_REDIS_CHANNEL = os.environ.get(
        "REALTIME_REDIS_CHANNEL", "mapv12:realtime"
    ).strip()
    ...
    DISPLAY_GNSS_STABLE_WINDOW_SEC = int(
        os.environ.get("DISPLAY_GNSS_STABLE_WINDOW_SEC", 25)
    )
    DISPLAY_GNSS_STABLE_DIST_M = float(os.environ.get("DISPLAY_GNSS_STABLE_DIST_M", 50))
    DISPLAY_GOOD_GNSS_MAX_ACC_M = float(
        os.environ.get("DISPLAY_GOOD_GNSS_MAX_ACC_M", 60)
    )
    ...
    DEVICE_STATUS_NO_SIGNAL_SEC = int(
        os.environ.get("DEVICE_STATUS_NO_SIGNAL_SEC", 600)
    )
    ...
    # Telegram bot token (для server-side авто-уведомлений,
    # например авто-отправка кнопки DutyTracker)
    # Фолбэк: если TELEGRAM_BOT_TOKEN не задан,
    # используем MAP_BOT_TOKEN / BOT_TOKEN (как в bot.py)
    TELEGRAM_BOT_TOKEN = (
    ...
    # Авто-отправка ссылки подключения DutyTracker при approve
    # (если TELEGRAM_BOT_TOKEN задан)
    MOBILE_CONNECT_AUTO_SEND = os.environ.get("MOBILE_CONNECT_AUTO_SEND", "1") == "1"

    # Предпочтительный base_url для bootstrap (если задан,
    # будет использоваться вместо авто-определения LAN IP)
    BOOTSTRAP_PREFERRED_BASE_URL = os.environ.get(
        "BOOTSTRAP_PREFERRED_BASE_URL", ""
    ).strip()

    # --- Telegram allow-lists (optional) ---
    # Список Telegram user_id (через запятую), которым разрешены
    # bootstrap-операции (выдача конфига для Android).
    ...
    # Если TELEGRAM_BOT_TOKEN + ADMIN_TELEGRAM_IDS заданы,
    # сервер может слать уведомления по CRIT алёртам.
    # 0/1
    TRACKER_TG_ALERT_NOTIFY = os.environ.get("TRACKER_TG_ALERT_NOTIFY", "1")
    # CSV: crit,warn,info (по умолчанию только crit)
    TRACKER_TG_NOTIFY_SEVERITIES = os.environ.get(
        "TRACKER_TG_NOTIFY_SEVERITIES", "crit"
    )
    # CSV: если задано — шлём только по указанным kind
    # (например 'tracking_off,app_error')
    TRACKER_TG_NOTIFY_KINDS = os.environ.get("TRACKER_TG_NOTIFY_KINDS", "")
    # Троттлинг: минимальный интервал между одинаковыми
    # уведомлениями по device+kind, сек
    TRACKER_TG_NOTIFY_MIN_INTERVAL_SEC = int(
        os.environ.get("TRACKER_TG_NOTIFY_MIN_INTERVAL_SEC", "900")
    )
    ...
    AUTO_REVOKE_ON_PAIR = (
        os.environ.get("AUTO_REVOKE_ON_PAIR", "1") or "1"
    ...
    # Если задан ключ - можно безопасно открыть /metrics наружу
    # (Prometheus) без allow_public.
    METRICS_API_KEY = os.environ.get("METRICS_API_KEY", "").strip()

    # --- Rate limits ---
    RATE_LIMIT_LOGIN_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_LOGIN_PER_MINUTE", "10")
    )
    RATE_LIMIT_CHAT_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_CHAT_PER_MINUTE", "120")
    )

    # Tracker (Android) – protects /api/tracker/* from accidental storms.
    # These limits are per-device (device.public_id).
    RATE_LIMIT_TRACKER_POINTS_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_TRACKER_POINTS_PER_MINUTE", "6000")
    )
    RATE_LIMIT_TRACKER_FINGERPRINTS_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_TRACKER_FINGERPRINTS_PER_MINUTE", "60")
    )
    RATE_LIMIT_TRACKER_HEALTH_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_TRACKER_HEALTH_PER_MINUTE", "120")
    )
    # SOS is intentionally low; window is 5 minutes in code.
    RATE_LIMIT_TRACKER_SOS_PER_5MIN = int(
        os.environ.get("RATE_LIMIT_TRACKER_SOS_PER_5MIN", "3")
    )

    # Admin API writes (objects/incidents). Per-admin (session) or per-IP fallback.
    RATE_LIMIT_OBJECTS_WRITE_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_OBJECTS_WRITE_PER_MINUTE", "120")
    )
    RATE_LIMIT_INCIDENTS_WRITE_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_INCIDENTS_WRITE_PER_MINUTE", "180")
    )
    RATE_LIMIT_OBJECTS_IMPORT_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_OBJECTS_IMPORT_PER_MINUTE", "10")
    )

    # Chat2 rate limits (unified knobs; keep backward compatible names)
    CHAT2_SEND_RATE_WINDOW_SEC = float(
        os.environ.get("CHAT2_SEND_RATE_WINDOW_SEC", "60")
    )
    CHAT2_SEND_RATE_LIMIT = int(
        os.environ.get("CHAT2_SEND_RATE_LIMIT", str(RATE_LIMIT_CHAT_PER_MINUTE))
    )
    CHAT2_UPLOAD_RATE_WINDOW_SEC = float(
        os.environ.get("CHAT2_UPLOAD_RATE_WINDOW_SEC", "60")
    )
    CHAT2_UPLOAD_RATE_LIMIT = int(
        os.environ.get(
            "CHAT2_UPLOAD_RATE_LIMIT",
            os.environ.get("RATE_LIMIT_MEDIA_UPLOAD_PER_MINUTE", "20"),
        )
    )
    ...
    RETENTION_DELETE_ONLY_CLOSED = (
        os.environ.get("RETENTION_DELETE_ONLY_CLOSED", "1") or "1"
    ).strip().lower() in {"1", "true", "yes", "y"}
    ...
    RETENTION_SCHEDULER_ENABLED = (
        os.environ.get("RETENTION_SCHEDULER_ENABLED", "0") == "1"
    )
    RETENTION_SCHEDULER_EVERY_MINUTES = int(
        os.environ.get("RETENTION_SCHEDULER_EVERY_MINUTES", "360")
    )
    # Start delay to avoid running immediately after boot (seconds)
    RETENTION_SCHEDULER_START_DELAY_SEC = int(
        os.environ.get("RETENTION_SCHEDULER_START_DELAY_SEC", "30")
    )
    # Best-effort Redis lock to prevent duplicate runs in multi-worker setups
    RETENTION_SCHEDULER_LOCK_KEY = os.environ.get(
        "RETENTION_SCHEDULER_LOCK_KEY", "mapv12:retention:lock"
    )
    RETENTION_SCHEDULER_LOCK_TTL_SEC = int(
        os.environ.get("RETENTION_SCHEDULER_LOCK_TTL_SEC", "600")
    )
    ...
    MAIL_USE_TLS = (os.environ.get("MAIL_USE_TLS", "1") or "1").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
    ...
    RATE_LIMIT_BOT_MARKERS_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_BOT_MARKERS_PER_MINUTE", "60")
    )
    RATE_LIMIT_BOT_STATUS_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_BOT_STATUS_PER_MINUTE", "180")
    )
    RATE_LIMIT_BOT_CANCEL_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_BOT_CANCEL_PER_MINUTE", "60")
    )
    RATE_LIMIT_BOT_MYREQ_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_BOT_MYREQ_PER_MINUTE", "120")
    )

    # Duty bot endpoints (best-effort)
    RATE_LIMIT_DUTY_SHIFT_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_SHIFT_PER_MINUTE", "120")
    )
    RATE_LIMIT_DUTY_CHECKIN_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_CHECKIN_PER_MINUTE", "240")
    )
    RATE_LIMIT_DUTY_LIVE_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_LIVE_PER_MINUTE", "600")
    )
    RATE_LIMIT_DUTY_TRACKING_STOP_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_TRACKING_STOP_PER_MINUTE", "60")
    )
    RATE_LIMIT_DUTY_SOS_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_SOS_PER_MINUTE", "30")
    )
    RATE_LIMIT_DUTY_BREAK_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_BREAK_PER_MINUTE", "60")
    )
    RATE_LIMIT_DUTY_SETUNIT_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_DUTY_SETUNIT_PER_MINUTE", "60")
    )
    ...
    CELERY_BEAT_SCHEDULE = {
        ...
        "weekend-ai-mutation": {
            ...
            "schedule": float(
                os.environ.get("AI_MUTATION_INTERVAL_SEC", str(7 * 24 * 3600))
            ),
        },
        "weekly-critical-ai-mutation": {
            "task": "app.tasks.mutation_testing.run_ai_mutation_on_critical_modules",
            "schedule": float(
                os.environ.get("AI_MUTATION_CRITICAL_INTERVAL_SEC", str(7 * 24 * 3600))
            ),
        },
    ...
    ENABLE_INTERNAL_SCHEDULERS = (
        os.environ.get("ENABLE_INTERNAL_SCHEDULERS", "0") == "1"
    )
...
class DevelopmentConfig(Config):
    ...
    DEBUG = True
    ENABLE_INTERNAL_SCHEDULERS = (
        os.environ.get("ENABLE_INTERNAL_SCHEDULERS", "1") == "1"
    )
