...
def _bot_rate_limit(
    bucket: str, limit_key: str, default_limit: int
    ...
    try:
        ...
        ok, _info = check_rate_limit(
            bucket=bucket, ident=ip, limit=limit, window_seconds=60
        )
        if not ok:
...
def _kpi_5m_for_session(
    session_id: Optional[int], now: datetime
    ...
    try:
        ...
        pts = (
            TrackingPoint.query.filter_by(session_id=session_id)
...
def _is_est_point(
    tp: Optional[TrackingPoint], meta: Optional[Dict[str, Any]] = None
) -> bool:
    if not tp:
        return False
...
def _select_display_point(session_id: int, now: datetime) -> Optional[TrackingPoint]:
    ...
    Problem: indoors GNSS may intermittently produce a 'newer' but wrong
    point that causes jumps.
    Solution: when we have a fresh estimate (wifi_est), keep it until GNSS
    becomes stably good.
    """
    try:
        pts: List[TrackingPoint] = (
            TrackingPoint.query.filter_by(session_id=session_id)
            .order_by(desc(TrackingPoint.ts))
            .limit(30)
            .all()
    ...
    try:
        stable_window_sec = float(
            current_app.config.get("DISPLAY_GNSS_STABLE_WINDOW_SEC", 25)
        )
    except Exception:
    ...
    if est_fresh and gnss_good:
        ...
        if len(recent_good) >= 2:
            try:
                d = haversine_m(
                    float(recent_good[0].lat),
                    float(recent_good[0].lon),
                    float(recent_good[1].lat),
                    float(recent_good[1].lon),
                )
            except Exception:
                d = 9999.0
            if d <= stable_dist_m:
...
def _compute_device_status(
    now: datetime, last_point_ts: Optional[datetime], heartbeat_ts: Optional[datetime]
) -> Dict[str, Any]:
    ...
    try:
        if last_point_ts and heartbeat_ts:
            last_update_ts = (
                last_point_ts if last_point_ts >= heartbeat_ts else heartbeat_ts
            )
        else:
...
def _get_active_shift(user_id: str) -> Optional[DutyShift]:
    return (
        DutyShift.query.filter_by(user_id=str(user_id), ended_at=None)
        .order_by(desc(DutyShift.started_at))
        .first()
    )


def _get_or_create_active_shift(
    user_id: str, unit_label: Optional[str] = None
) -> DutyShift:
    ...
    _log_event(
        ...
        "SHIFT_AUTO_START",
        actor="system",
        payload={"reason": "no_active_shift"},
    )
    return sh
...
def _get_last_location(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    uid = str(user_id)
    sess = (
        TrackingSession.query.filter_by(user_id=uid, ended_at=None)
    ...
    lp = (
        TrackingPoint.query.filter_by(user_id=uid)
        .order_by(desc(TrackingPoint.ts))
        .first()
    )
    if lp:
...
def _ensure_tracks_dir() -> str:
    up = current_app.config.get("UPLOAD_FOLDER") or os.path.join(
...
def _build_route_svg(
    ...
    if not points:
        return (
            ...
            f"height='{height}'><text x='20' y='40'>Нет данных</text></svg>"
        )
    ...
    if not lats or not lons:
        return (
            f"<svg xmlns='http://www.w3.org/2000/svg' width='{width}' "
            f"height='{height}'><text x='20' y='40'>Нет координат</text></svg>"
        )
    ...
    for st in stops:
        ...
        stop_circles.append(
            f"<circle cx='{x:.2f}' cy='{y:.2f}' r='7' fill='orange' "
            "stroke='black' stroke-width='1'/>"
            f"<text x='{x + 10:.2f}' y='{y + 4:.2f}' font-size='12' "
    ...
    <circle cx='{sx:.2f}' cy='{sy:.2f}' r='8' fill='green'
            stroke='black' stroke-width='1'/>
    <circle cx='{ex:.2f}' cy='{ey:.2f}' r='8' fill='red'
            stroke='black' stroke-width='1'/>
    {"".join(stop_circles)}
    <text x='20' y='{height - 15}' font-size='12' fill='#444'>
...
def _compute_stops(
    points: List[TrackingPoint], radius_m: float = 10.0, min_sec: int = 60
) -> List[TrackingStop]:
...
def _create_notification(
    user_id: str, kind: str, text: str, payload: Optional[Dict[str, Any]] = None
) -> None:
    n = DutyNotification(
        user_id=str(user_id),
        created_at=_utcnow(),
...
def admin_problems_page():
    ...
    Техническая страница: диспетчер видит активные алёрты
    (stale/queue/battery/gps…) и может ACK/CLOSE.
    """
    require_admin("viewer")
...
def api_bot_shift_start():
    ...
    _log_event(
        ...
        "SHIFT_START",
        actor="user",
        payload={"lat": lat, "lon": lon, "unit_label": unit},
    )
    db.session.commit()
...
def api_bot_shift_end():
    ...
    if not sh:
        return jsonify(
            {"ok": True, "shift_id": None, "message": "no active shift"}
        ), 200
    ...
    sh.end_lon = lon
    _log_event(
        user_id, sh.id, "SHIFT_END", actor="user", payload={"lat": lat, "lon": lon}
    )
    # Закрываем активный трекинг (если есть)
    sess = (
        TrackingSession.query.filter_by(user_id=user_id, ended_at=None)
        .order_by(desc(TrackingSession.started_at))
        .first()
    )
    if sess:
        ...
        _log_event(
            ...
            "TRACKING_AUTO_STOP",
            actor="system",
            payload={"session_id": sess.id, "reason": "shift_end"},
        )
    db.session.commit()
...
def api_bot_checkin():
    ...
    broadcast_event_sync(
        "checkin",
        {"user_id": user_id, "shift_id": sh.id, "lat": lat, "lon": lon, "note": note},
    )
    return jsonify({"ok": True, "shift_id": sh.id})
...
def api_bot_live_location():
    ...
    if is_live and message_id is not None:
        sess = TrackingSession.query.filter_by(
            user_id=user_id, message_id=int(message_id), ended_at=None
        ).first()
    if is_live and sess is None:
        ...
        _log_event(
            ...
            "TRACKING_START",
            actor="user",
            payload={"session_id": sess.id, "message_id": message_id},
        )
        broadcast_event_sync(
            "tracking_started",
            {
    ...
    pt = TrackingPoint(
        ...
        lon=lon,
        accuracy_m=data.get("accuracy_m"),
        kind="live" if is_live else "location",
        raw_json=json.dumps(data, ensure_ascii=False),
    )
    db.session.add(pt)
    ...
    _log_event(
        user_id,
        sh.id,
        "TRACKING_POINT" if is_live else "LOCATION_POINT",
        actor="user",
        payload={"session_id": sess.id if sess else None, "lat": lat, "lon": lon},
    )
    ...
    return jsonify(
        {"ok": True, "shift_id": sh.id, "session_id": sess.id if sess else None}
    )
...
def api_bot_tracking_stop():
    ...
    if bad:
        return jsonify(bad[0]), bad[1]
    bad = _bot_rate_limit(
        "duty_tracking_stop", "RATE_LIMIT_DUTY_TRACKING_STOP_PER_MINUTE", 60
    )
    if bad:
        return jsonify(bad[0]), bad[1]
    data = request.get_json(silent=True) or {}
    ...
    # точки сессии
    points = (
        TrackingPoint.query.filter_by(session_id=sess.id)
    ...
    _log_event(
        user_id,
        sh.id if sh else None,
        "TRACKING_STOP",
        actor="user",
        payload={"session_id": sess.id, "snapshot": filename, **summary},
    )
    db.session.commit()
    ...
    broadcast_event_sync(
        "tracking_stopped",
        {
            ...
            "session_id": sess.id,
            "snapshot_url": f"/uploads/{filename}",
        },
    )
    return jsonify(
        {
...
def api_bot_sos():
    ...
    _log_event(
        ...
        "SOS_CREATED",
        actor="user",
        payload={"sos_id": alert.id, "lat": alert.lat, "lon": alert.lon},
    )
    db.session.commit()
...
def api_bot_break_request():
    ...
    _log_event(
        ...
        "BREAK_REQUEST",
        actor="user",
        payload={"break_id": br.id, "duration_min": duration},
    )
    db.session.commit()
...
def api_admin_dashboard():
    ...
    res_shifts = []
    for sh in shifts:
        # last point: prefer last TrackingPoint (has accuracy_m).
        # Fallback to session.last_*.
        sess = (
            TrackingSession.query.filter_by(user_id=sh.user_id, ended_at=None)
            .order_by(desc(TrackingSession.started_at))
            .first()
        )
        last = None
        ...
        if not last:
            lp = (
                TrackingPoint.query.filter_by(user_id=sh.user_id)
                .order_by(desc(TrackingPoint.ts))
                .first()
            )
            if lp and lp.lat is not None and lp.lon is not None:
                ...
                if meta and last:
                    last.update(meta)
        br = (
            BreakRequest.query.filter_by(user_id=sh.user_id)
            .filter(BreakRequest.status.in_(["requested", "started"]))
            .order_by(desc(BreakRequest.requested_at))
            .first()
        )
        ...
        # иначе Dashboard может показывать старую health/"нет связи"
        ...
        h = (
            TrackerDeviceHealth.query.filter_by(device_id=dev.public_id).first()
            if dev
            else (
                TrackerDeviceHealth.query.filter_by(user_id=sh.user_id)
                .order_by(desc(TrackerDeviceHealth.updated_at))
                .first()
            )
        )
        ...
        try:
            if h and h.updated_at:
                heartbeat_ts = (
                    heartbeat_ts
                    if (heartbeat_ts and heartbeat_ts >= h.updated_at)
                    else h.updated_at
                )
        except Exception:
            pass
        ...
        res_shifts.append(
            {
                ...
                "kpi_5m": _kpi_5m_for_session(
                    last.get("session_id") if last else None, now
                ),
            }
        )

    breaks = (
        BreakRequest.query.filter(BreakRequest.status.in_(["requested", "started"]))
        .order_by(desc(BreakRequest.requested_at))
        .all()
    )
    ...
    sos_rows = (
        SosAlert.query.filter(SosAlert.status.in_(["open", "acked"]))
    ...
    return jsonify(
        {
            ...
            "active_shifts": res_shifts,
            "breaks": res_breaks,
            "sos_active": res_sos,
            "sos_active_count": len(res_sos),
        }
    )
...
def api_admin_shift_detail(shift_id: int):
    ...
    # последние сессии трекинга по этой смене (или по user_id,
    ...
    ev = (
        DutyEvent.query.filter_by(shift_id=sh.id)
        .order_by(DutyEvent.ts.asc())
        .limit(200)
        .all()
    )
    ...
    sos_active = (
        SosAlert.query.filter_by(user_id=sh.user_id)
        .filter(SosAlert.status.in_(["open", "acked"]))
        .order_by(desc(SosAlert.created_at))
        .first()
    )
    sos_last = (
...
def api_admin_sos_ack(sos_id: int):
    ...
    if row.status != "acked":
        ...
        _log_event(
            ...
            "SOS_ACKED",
            actor="admin",
            payload={"sos_id": row.id},
        )
        _create_notification(
            row.user_id,
            "sos_acked",
            "✅ SOS принят оператором. Держитесь, помощь в пути.",
            {"sos_id": row.id},
        )
        db.session.commit()
...
def api_admin_sos_close(sos_id: int):
    ...
    if row.status != "closed":
        ...
        _create_notification(
            row.user_id,
            "sos_closed",
            "🟢 SOS закрыт оператором. Спасибо за подтверждение.",
            {"sos_id": row.id},
        )
        db.session.commit()
...
def api_admin_tracking(session_id: int):
    ...
    stops = (
        TrackingStop.query.filter_by(session_id=session_id)
        .order_by(TrackingStop.start_ts.asc())
        .all()
    )
    return jsonify(
        {
            ...
            "snapshot_url": f"/uploads/{sess.snapshot_path}"
            if sess.snapshot_path
            else None,
        }
    )
...
def api_admin_break_approve(break_id: int):
    ...
    _log_event(
        ...
        "BREAK_START",
        actor="admin",
        payload={"break_id": br.id, "ends_at": br.ends_at.isoformat()},
    )
    _create_notification(
        br.user_id,
        "break_started",
        (
            f"🍽 Обед подтверждён. Длительность: {br.duration_min} мин. "
            f"Конец: {br.ends_at.strftime('%H:%M:%S')} (UTC)."
        ),
        {"break_id": br.id, "ends_at": br.ends_at.isoformat()},
    )
    db.session.commit()
...
def api_admin_break_end(break_id: int):
    ...
    br.ended_at = _utcnow()
    _log_event(
        br.user_id, br.shift_id, "BREAK_END", actor="admin", payload={"break_id": br.id}
    )
    _create_notification(
        br.user_id,
        "break_ended",
        "✅ Обед завершён оператором. Возвращайтесь к несению службы.",
        {"break_id": br.id},
    )
    db.session.commit()
    ...
    broadcast_event_sync(
        "break_ended",
        {"user_id": br.user_id, "shift_id": br.shift_id, "break_id": br.id},
    )
    return jsonify({"ok": True, "break": br.to_dict()})
...
def api_notify_targets():
    ...
    if bad:
        return jsonify(bad[0]), bad[1]
    rows = (
        DutyNotification.query.filter_by(acked=False)
        .order_by(DutyNotification.created_at.asc())
        .all()
    )
...
def api_user_pending(user_id: str):
    ...
    if bad:
        return jsonify(bad[0]), bad[1]
    rows = (
        DutyNotification.query.filter_by(user_id=str(user_id), acked=False)
        .order_by(DutyNotification.created_at.asc())
        .limit(20)
        .all()
    )
    ...
    for br in due:
        if br.ends_at and br.ends_at <= now:
            ...
            _create_notification(
                br.user_id,
                "break_due",
                "⏱ Время обеда истекло. Ожидайте отметки оператора.",
                {"break_id": br.id},
            )
            changed += 1
    if changed:
