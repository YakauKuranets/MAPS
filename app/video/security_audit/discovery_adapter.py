# -*- coding: utf-8 -*-
"""Адаптер для обнаружения камер с использованием
AutoDiscoveryService и fallback сканера."""
...
async def detect_camera_comprehensive(
    ...
    if result and result.channels:
        ...
        logger.info(
            "AutoDiscoveryService нашёл камеру %s:%s (vendor=%s, prober=%s)",
            ip,
            port,
            vendor,
            result.prober,
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for port in CAMERA_PORTS:
            if port not in [80, 443, 8080, 8443]:
                if await scan_port(ip, port, timeout):
                    logger.info(
                        f"Найден открытый порт {port} на {ip}, но вендор не определён"
                    )
