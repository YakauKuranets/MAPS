...
def _cameras_from_row(row: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 1) compact
    compact = (
        (row.get("cameras") or "").strip()
        if isinstance(row.get("cameras"), str)
        else row.get("cameras")
    )
    ...
    # 2) single camera
    cu = (
        (row.get("camera_url") or "").strip()
        if isinstance(row.get("camera_url"), str)
        else ""
    )
    if cu:
        out.append(
...
def _query_objects(q: str, tag: str):
    ...
    if q:
        like = f"%{q}%"
        query = query.filter(
            (Object.name.ilike(like)) | (Object.description.ilike(like))
        )
    if tag:
        like = f"%{tag}%"
...
def list_objects() -> Any:
    ...
    lite = (request.args.get("lite") or "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def create_object() -> Any:
    ...
    rl = _rate_limit_or_429(
        "objects_write",
        current_app.config.get("RATE_LIMIT_OBJECTS_WRITE_PER_MINUTE", 120),
        60,
    )
    if rl is not None:
        return rl
    ...
    for cam in cams:
        ...
        obj.cameras.append(
            ObjectCamera(url=url, label=label or None, type=ctype or None)
        )
...
def import_objects() -> Any:
    ...
    rl = _rate_limit_or_429(
        "objects_import",
        current_app.config.get("RATE_LIMIT_OBJECTS_IMPORT_PER_MINUTE", 10),
        60,
    )
    if rl is not None:
        return rl
...
def _read_xlsx_rows(file) -> List[Dict[str, Any]]:
    ...
    for row_values in rows_iter:
        ...
        # skip fully empty
        if all(
            (v is None or (isinstance(v, str) and not v.strip())) for v in row.values()
        ):
            continue
...
def _upsert_object_from_row(row: Dict[str, Any]) -> str:
    ...
    name = (
        (row.get("name") or row.get("address") or "").strip()
        if isinstance(row.get("name") or row.get("address") or "", str)
        else (row.get("name") or row.get("address") or "")
    )
