...
from .middlewares.telegram_webapp_security import (
    enforce_telegram_init_data,
...
def _schedule_ai_vision_for_pending(
    pending_id: int, photo_value: Optional[str]
) -> None:
    ...
    Thread(
        target=_run_ai_vision_job, args=(app_obj, pending_id, image_path), daemon=True
    ).start()
...
def bot_voice_camera_submit() -> Response:
    ...
    # photo может прийти как base64-строка либо как файл
    # (обрабатывается на следующих шагах пайплайна)
        ...
        return jsonify(
            {
                ...
                "photo_mode": "base64"
                if photo_b64
                else ("file" if photo_file else None),
            }
        ), 200
    except Exception as exc:
        current_app.logger.exception("voice_camera_submit failed")
        return jsonify(
            {"error": "voice_camera_submit_failed", "details": str(exc)}
        ), 500
    finally:
        try:
...
def find_duplicate_db(
    name: str, lat: Optional[float], lon: Optional[float], threshold_m: int = 100
...
def add_marker_from_bot() -> Response:
    ...
    try:
        ...
        ok, _info = check_rate_limit(
            bucket="bot_markers", ident=ip, limit=limit, window_seconds=60
        )
        if not ok:
    ...
    # то в pending_markers/addresses может не быть zone_id.
    # Это приводит к 500 при INSERT.
    # Мягко добавляем недостающие колонки для SQLite.
    try:
        ensure_sqlite_schema_minimal()
    except Exception:
        current_app.logger.exception(
            "ensure_sqlite_schema_minimal failed in /api/bot/markers"
        )
    # разбор тела запроса
    if request.files:
    ...
    # Идемпотентность: если пришли user_id+message_id
    # и такая заявка уже есть — возвращаем её.
    if user_id and message_id:
        try:
            existing = PendingMarker.query.filter_by(
        ...
        if existing:
            return jsonify(
                {"pending": existing.id, "status": "pending", "existing": True}
            ), 200
    ...
    if not in_range(lat, lon):
                ...
                if offline_path and os.path.isfile(offline_path):
                    with open(offline_path, "r", encoding="utf-8") as fh:
                        ...
                        for entry in offline_data:
                            disp = (
                                entry.get("display_name") or entry.get("address") or ""
                            ).lower()
        ...
        if coords:
            ...
            return jsonify(
                {"error": "Invalid or missing coordinates for geocoding"}
            ), 400
    # финальная проверка координат
    if not in_range(lat, lon):
        return jsonify({"error": "Invalid coordinates"}), 400
    # проверка на дубликаты: включено по умолчанию,
    # отключается ?dedupe=0, форсируется ?force=1
    ...
    try:
        ...
        msg = str(e).lower()
        if (
            "pending_markers" in msg
            and "zone_id" in msg
            and ("no column named" in msg or "has no column named" in msg)
        ):
            db.session.rollback()
            try:
                ensure_sqlite_schema_minimal()
                ...
                current_app.logger.exception(
                    "ensure_sqlite_schema_minimal retry failed in /api/bot/markers"
                )
            try:
                pending = _insert_pending()
            except Exception:
                ...
                return jsonify(
                    {
                        ...
                        "message": (
                            "База данных устарела (нет zone_id). "
    ...
    hist = PendingHistory(pending_id=pid, status="pending")
    db.session.add(hist)
    db.session.commit()
...
def bot_webapp_submit() -> Response:
    ...
    lat = parse_coord(
        (payload.get("coords") or {}).get("lat")
        if isinstance(payload.get("coords"), dict)
        else payload.get("lat")
    )
    lon = parse_coord(
        (payload.get("coords") or {}).get("lon")
        if isinstance(payload.get("coords"), dict)
    ...
    pending = PendingMarker(
        ...
        category=category,
        photo=photo,
        reporter=(user_data.get("last_name") or user_data.get("username") or "").strip()
        or None,
...
def bot_marker_status(pid: int) -> Response:
    ...
    try:
        ...
        ok, _info = check_rate_limit(
            bucket="bot_marker_status", ident=ip, limit=limit, window_seconds=60
        )
        if not ok:
            return jsonify({"error": "rate_limited"}), 429
    except Exception:
    ...
    rec: Optional[PendingHistory] = (
        PendingHistory.query.filter_by(pending_id=pid)
...
def bot_marker_cancel(pid: int) -> Response:
    ...
    try:
        ...
        ok, _info = check_rate_limit(
            bucket="bot_marker_cancel", ident=ip, limit=limit, window_seconds=60
        )
        if not ok:
            return jsonify({"error": "rate_limited"}), 429
    except Exception:
    ...
    hist = PendingHistory(pending_id=pid, status="cancelled")
    db.session.add(hist)
    db.session.commit()
...
def bot_my_requests(user_id: str) -> Response:
    ...
    try:
        ...
        ok, _info = check_rate_limit(
            bucket="bot_my_requests", ident=ip, limit=limit, window_seconds=60
        )
        if not ok:
            return jsonify({"error": "rate_limited"}), 429
    except Exception:
    ...
    histories: List[PendingHistory] = PendingHistory.query.filter(
        PendingHistory.pending_id.in_(ids)
    ).all()
