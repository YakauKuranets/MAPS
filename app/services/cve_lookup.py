...
def find_cves_for_product(
    product_hint: str,
    limit: int = 20,
) -> list[dict[str, Any]]:
    """Find CVE entries by product/vendor hint in affected_products JSON.

    Also searches in CVE description.
    """
    needle = (product_hint or "").strip().lower()
    if not needle:
        return []

    # JSON filtering is backend-dependent;
    # do broad filter then in-Python match for portability.
    rows = (
        CVE.query.order_by(CVE.last_updated.desc().nullslast())
        .limit(1000)
        .all()
    )
