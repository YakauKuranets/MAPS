...
def _run_retention(app: Flask, t: Thresholds) -> None:
    ...
    try:
        ...
        # points
        db.session.query(TrackingPoint).filter(TrackingPoint.ts < cutoff_points).delete(
            synchronize_session=False
        )
        # old ended sessions snapshots are handled elsewhere; this is minimal

        cutoff_health_log = _utcnow() - timedelta(
            days=max(7, int(t.retention_health_log_days))
        )
        try:
            from ..models import TrackerDeviceHealthLog

            db.session.query(TrackerDeviceHealthLog).filter(
                TrackerDeviceHealthLog.ts < cutoff_health_log
        ...
        db.session.commit()
        app.logger.info(
            "tracker retention done: points<%s, health_log<%s",
            cutoff_points.isoformat(),
            cutoff_health_log.isoformat(),
        )
    except Exception:
        db.session.rollback()
...
def _last_point_ts(user_id: str) -> Optional[datetime]:
    ...
    p = (
        TrackingPoint.query.join(
            TrackingSession, TrackingPoint.session_id == TrackingSession.id
        )
...
def _upsert_alert(
    *,
    ...
    row = (
        TrackerAlert.query.filter_by(
            device_id=device_id, user_id=user_id, kind=kind, is_active=True
        )
    ...
    if not row:
        ...
        # (чтобы не спамить WS/БД каждый тик)
        if (
            (row.severity != severity)
            or (row.message != message)
            or ((row.payload_json or "") != (payload_json or ""))
        ):
            row.severity = severity
            row.message = message
...
def _run_once(app: Flask, t: Thresholds) -> None:
    ...
    for d in devices:
        ...
        if h:
            if h.battery_pct is not None and (h.is_charging is not True):
                if h.battery_pct <= t.battery_crit:
                    ...
                    if _close_alert(d.public_id, d.user_id, "battery_low"):
                        changes.append(
                            ("closed", ("battery_low", d.public_id, d.user_id))
                        )
            else:
                if _close_alert(d.public_id, d.user_id, "battery_low"):
            ...
            if h.queue_size is not None:
                if h.queue_size >= t.queue_crit:
                    ...
                    if _close_alert(d.public_id, d.user_id, "queue_growing"):
                        changes.append(
                            ("closed", ("queue_growing", d.public_id, d.user_id))
                        )
                ...
                if acc is not None:
                    if acc >= float(t.accuracy_crit_m):
                        al, created, changed = _upsert_alert(
                            ...
                            payload={
                                "accuracy_m": acc,
                                "warn_m": t.accuracy_warn_m,
                                "crit_m": t.accuracy_crit_m,
                            },
                        )
                        if created:
                            changes.append(("created", al))
                        ...
                        if _close_alert(d.public_id, d.user_id, "low_accuracy"):
                            changes.append(
                                ("closed", ("low_accuracy", d.public_id, d.user_id))
                            )
            else:
                if _close_alert(d.public_id, d.user_id, "low_accuracy"):
            ...
            try:
                active_shift = DutyShift.query.filter_by(
                    user_id=d.user_id, ended_at=None
                ).first()
            except Exception:
                active_shift = None
            if active_shift and (h.tracking_on is False):
                al, created, changed = _upsert_alert(
                    ...
                    payload={
                        "tracking_on": h.tracking_on,
                        "shift_id": getattr(active_shift, "id", None),
                    },
                )
                if created:
                    changes.append(("created", al))
        ...
        try:
            if kind in ("created", "updated") and isinstance(data, TrackerAlert):
                ...
                broadcast_event_sync(
                    ...
                    {"kind": akind, "device_id": device_id, "user_id": user_id},
                )
        except Exception:
            pass
