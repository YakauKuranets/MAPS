...
def _recommendations_for_alert(alert: TrackerAlert) -> List[str]:
    # Мини-копия логики из static/js/recs.js
    # — только самые полезные пункты для Telegram.
...
def _should_notify(alert: TrackerAlert) -> bool:
    ...
    severities = _parse_csv_set(
        str(current_app.config.get("TRACKER_TG_NOTIFY_SEVERITIES", "crit"))
    )
...
def _throttle_ok(device_id: Optional[str], kind: str, chat_id: str) -> bool:
    min_sec = int(
        current_app.config.get("TRACKER_TG_NOTIFY_MIN_INTERVAL_SEC", 900) or 900
    )
    if min_sec <= 0:
        return True
    cutoff = _now_utc() - timedelta(seconds=min_sec)
    last = (
        TrackerAlertNotifyLog.query.filter_by(
            device_id=device_id, kind=kind, sent_to=str(chat_id)
        )
        ...
        for chat_id in admin_ids:
            try:
                ...
                payload = send_telegram_message(
                    token, str(chat_id), text, disable_web_preview=True, timeout_sec=8
                )
