...
def upgrade():
    ...
    if not _table_exists(conn, "duty_events"):
        op.create_table(
            ...
            sa.Column(
                "shift_id", sa.Integer(), sa.ForeignKey("duty_shifts.id"), nullable=True
            ),
    ...
    if _table_exists(conn, "duty_shifts") and not _index_exists(
        conn, "duty_shifts", "ix_duty_shifts_started_at"
    ):
        op.create_index("ix_duty_shifts_started_at", "duty_shifts", ["started_at"])
    if _table_exists(conn, "duty_shifts") and not _index_exists(
        conn, "duty_shifts", "ix_duty_shifts_ended_at"
    ):
        op.create_index("ix_duty_shifts_ended_at", "duty_shifts", ["ended_at"])
    if _table_exists(conn, "duty_events") and not _index_exists(
        conn, "duty_events", "ix_duty_events_user_id"
    ):
        op.create_index("ix_duty_events_user_id", "duty_events", ["user_id"])
    if _table_exists(conn, "duty_events") and not _index_exists(
        conn, "duty_events", "ix_duty_events_shift_id"
    ):
        op.create_index("ix_duty_events_shift_id", "duty_events", ["shift_id"])
    if _table_exists(conn, "duty_events") and not _index_exists(
        conn, "duty_events", "ix_duty_events_ts"
    ):
        op.create_index("ix_duty_events_ts", "duty_events", ["ts"])
    if _table_exists(conn, "duty_events") and not _index_exists(
        conn, "duty_events", "ix_duty_events_event_type"
    ):
        op.create_index("ix_duty_events_event_type", "duty_events", ["event_type"])
    if _table_exists(conn, "tracking_sessions") and not _index_exists(
        conn, "tracking_sessions", "ix_tracking_sessions_user_id"
    ):
        op.create_index(
            "ix_tracking_sessions_user_id", "tracking_sessions", ["user_id"]
        )
    if _table_exists(conn, "tracking_sessions") and not _index_exists(
        conn, "tracking_sessions", "ix_tracking_sessions_shift_id"
    ):
        op.create_index(
            "ix_tracking_sessions_shift_id", "tracking_sessions", ["shift_id"]
        )
    if _table_exists(conn, "tracking_sessions") and not _index_exists(
        conn, "tracking_sessions", "ix_tracking_sessions_started_at"
    ):
        op.create_index(
            "ix_tracking_sessions_started_at", "tracking_sessions", ["started_at"]
        )
    if _table_exists(conn, "tracking_sessions") and not _index_exists(
        conn, "tracking_sessions", "ix_tracking_sessions_ended_at"
    ):
        op.create_index(
            "ix_tracking_sessions_ended_at", "tracking_sessions", ["ended_at"]
        )
    if _table_exists(conn, "tracking_points") and not _index_exists(
        conn, "tracking_points", "ix_tracking_points_session_id"
    ):
        op.create_index(
            "ix_tracking_points_session_id", "tracking_points", ["session_id"]
        )
    if _table_exists(conn, "tracking_points") and not _index_exists(
        conn, "tracking_points", "ix_tracking_points_user_id"
    ):
        op.create_index("ix_tracking_points_user_id", "tracking_points", ["user_id"])
    if _table_exists(conn, "tracking_points") and not _index_exists(
        conn, "tracking_points", "ix_tracking_points_ts"
    ):
        op.create_index("ix_tracking_points_ts", "tracking_points", ["ts"])
    if _table_exists(conn, "tracking_stops") and not _index_exists(
        conn, "tracking_stops", "ix_tracking_stops_session_id"
    ):
        op.create_index(
            "ix_tracking_stops_session_id", "tracking_stops", ["session_id"]
        )
    if _table_exists(conn, "tracking_stops") and not _index_exists(
        conn, "tracking_stops", "ix_tracking_stops_start_ts"
    ):
        op.create_index("ix_tracking_stops_start_ts", "tracking_stops", ["start_ts"])
    if _table_exists(conn, "tracking_stops") and not _index_exists(
        conn, "tracking_stops", "ix_tracking_stops_end_ts"
    ):
        op.create_index("ix_tracking_stops_end_ts", "tracking_stops", ["end_ts"])
    if _table_exists(conn, "break_requests") and not _index_exists(
        conn, "break_requests", "ix_break_requests_user_id"
    ):
        op.create_index("ix_break_requests_user_id", "break_requests", ["user_id"])
    if _table_exists(conn, "break_requests") and not _index_exists(
        conn, "break_requests", "ix_break_requests_shift_id"
    ):
        op.create_index("ix_break_requests_shift_id", "break_requests", ["shift_id"])
    if _table_exists(conn, "break_requests") and not _index_exists(
        conn, "break_requests", "ix_break_requests_requested_at"
    ):
        op.create_index(
            "ix_break_requests_requested_at", "break_requests", ["requested_at"]
        )
    if _table_exists(conn, "sos_alerts") and not _index_exists(
        conn, "sos_alerts", "ix_sos_alerts_user_id"
    ):
        op.create_index("ix_sos_alerts_user_id", "sos_alerts", ["user_id"])
    if _table_exists(conn, "sos_alerts") and not _index_exists(
        conn, "sos_alerts", "ix_sos_alerts_shift_id"
    ):
        op.create_index("ix_sos_alerts_shift_id", "sos_alerts", ["shift_id"])
    if _table_exists(conn, "sos_alerts") and not _index_exists(
        conn, "sos_alerts", "ix_sos_alerts_session_id"
    ):
        op.create_index("ix_sos_alerts_session_id", "sos_alerts", ["session_id"])
    if _table_exists(conn, "sos_alerts") and not _index_exists(
        conn, "sos_alerts", "ix_sos_alerts_created_at"
    ):
        op.create_index("ix_sos_alerts_created_at", "sos_alerts", ["created_at"])
    if _table_exists(conn, "duty_notifications") and not _index_exists(
        conn, "duty_notifications", "ix_duty_notifications_user_id"
    ):
        op.create_index(
            "ix_duty_notifications_user_id", "duty_notifications", ["user_id"]
        )
    if _table_exists(conn, "duty_notifications") and not _index_exists(
        conn, "duty_notifications", "ix_duty_notifications_created_at"
    ):
        op.create_index(
            "ix_duty_notifications_created_at", "duty_notifications", ["created_at"]
        )
    if _table_exists(conn, "duty_notifications") and not _index_exists(
        conn, "duty_notifications", "ix_duty_notifications_kind"
    ):
        op.create_index("ix_duty_notifications_kind", "duty_notifications", ["kind"])
    if _table_exists(conn, "duty_notifications") and not _index_exists(
        conn, "duty_notifications", "ix_duty_notifications_acked"
    ):
        op.create_index("ix_duty_notifications_acked", "duty_notifications", ["acked"])
    if _table_exists(conn, "tracker_pair_codes") and not _index_exists(
        conn, "tracker_pair_codes", "ix_tracker_pair_codes_created_at"
    ):
        op.create_index(
            "ix_tracker_pair_codes_created_at", "tracker_pair_codes", ["created_at"]
        )
    if _table_exists(conn, "tracker_pair_codes") and not _index_exists(
        conn, "tracker_pair_codes", "ix_tracker_pair_codes_expires_at"
    ):
        op.create_index(
            "ix_tracker_pair_codes_expires_at", "tracker_pair_codes", ["expires_at"]
        )
    if _table_exists(conn, "tracker_pair_codes") and not _index_exists(
        conn, "tracker_pair_codes", "ix_tracker_pair_codes_used_at"
    ):
        op.create_index(
            "ix_tracker_pair_codes_used_at", "tracker_pair_codes", ["used_at"]
        )
    if _table_exists(conn, "tracker_devices") and not _index_exists(
        conn, "tracker_devices", "ix_tracker_devices_created_at"
    ):
        op.create_index(
            "ix_tracker_devices_created_at", "tracker_devices", ["created_at"]
        )
    if _table_exists(conn, "tracker_devices") and not _index_exists(
        conn, "tracker_devices", "ix_tracker_devices_last_seen_at"
    ):
        op.create_index(
            "ix_tracker_devices_last_seen_at", "tracker_devices", ["last_seen_at"]
        )
    if _table_exists(conn, "tracker_devices") and not _index_exists(
        conn, "tracker_devices", "ix_tracker_devices_is_revoked"
    ):
        op.create_index(
            "ix_tracker_devices_is_revoked", "tracker_devices", ["is_revoked"]
        )
    if _table_exists(conn, "tracker_devices") and not _index_exists(
        conn, "tracker_devices", "ix_tracker_devices_user_id"
    ):
        op.create_index("ix_tracker_devices_user_id", "tracker_devices", ["user_id"])
    if _table_exists(conn, "tracker_device_health") and not _index_exists(
        conn, "tracker_device_health", "ix_tracker_device_health_user_id"
    ):
        op.create_index(
            "ix_tracker_device_health_user_id", "tracker_device_health", ["user_id"]
        )
    if _table_exists(conn, "tracker_device_health") and not _index_exists(
        conn, "tracker_device_health", "ix_tracker_device_health_updated_at"
    ):
        op.create_index(
            "ix_tracker_device_health_updated_at",
            "tracker_device_health",
            ["updated_at"],
        )
    if _table_exists(conn, "tracker_device_health_log") and not _index_exists(
        conn, "tracker_device_health_log", "ix_tracker_device_health_log_user_id"
    ):
        op.create_index(
            "ix_tracker_device_health_log_user_id",
            "tracker_device_health_log",
            ["user_id"],
        )
    if _table_exists(conn, "tracker_device_health_log") and not _index_exists(
        conn, "tracker_device_health_log", "ix_tracker_device_health_log_ts"
    ):
        op.create_index(
            "ix_tracker_device_health_log_ts", "tracker_device_health_log", ["ts"]
        )
    if _table_exists(conn, "tracker_alerts") and not _index_exists(
        conn, "tracker_alerts", "ix_tracker_alerts_user_id"
    ):
        op.create_index("ix_tracker_alerts_user_id", "tracker_alerts", ["user_id"])
    if _table_exists(conn, "tracker_alerts") and not _index_exists(
        conn, "tracker_alerts", "ix_tracker_alerts_created_at"
    ):
        op.create_index(
            "ix_tracker_alerts_created_at", "tracker_alerts", ["created_at"]
        )
    if _table_exists(conn, "tracker_alerts") and not _index_exists(
        conn, "tracker_alerts", "ix_tracker_alerts_updated_at"
    ):
        op.create_index(
            "ix_tracker_alerts_updated_at", "tracker_alerts", ["updated_at"]
        )
    if _table_exists(conn, "tracker_alerts") and not _index_exists(
        conn, "tracker_alerts", "ix_tracker_alerts_is_active"
    ):
        op.create_index("ix_tracker_alerts_is_active", "tracker_alerts", ["is_active"])
    if _table_exists(conn, "tracker_admin_audit") and not _index_exists(
        conn, "tracker_admin_audit", "ix_tracker_admin_audit_ts"
    ):
...
def downgrade():
    ...
    for table_name, index_name in [
        ...
        if _table_exists(conn, table_name) and _index_exists(
            conn, table_name, index_name
        ):
