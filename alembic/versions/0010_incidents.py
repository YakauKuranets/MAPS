...
def upgrade():
    ...
    if not _table_exists(conn, "incidents"):
        op.create_table(
            ...
            sa.Column(
                "status", sa.String(length=32), nullable=False, server_default="new"
            ),
    ...
    if not _table_exists(conn, "incident_events"):
        ...
        op.create_index(
            "ix_incident_events_incident", "incident_events", ["incident_id"]
        )
    ...
    if not _table_exists(conn, "incident_assignments"):
        ...
        op.create_index(
            "ix_incident_assignments_incident", "incident_assignments", ["incident_id"]
        )
        op.create_index(
            "ix_incident_assignments_shift", "incident_assignments", ["shift_id"]
        )
        # FKs
        op.create_foreign_key(
...
def downgrade():
    ...
    if _table_exists(conn, "incident_assignments"):
        if _index_exists(
            conn, "incident_assignments", "ix_incident_assignments_incident"
        ):
            op.drop_index(
                "ix_incident_assignments_incident", table_name="incident_assignments"
            )
        if _index_exists(conn, "incident_assignments", "ix_incident_assignments_shift"):
            op.drop_index(
                "ix_incident_assignments_shift", table_name="incident_assignments"
            )
