...
async def extract_social_profiles(
    alias: str, browser_context: Any
) -> list[dict[str, Any]]:
    ...
    for platform in platforms:
        try:
            logger.info(
                "[SOCMINT] Checking %s for alias %s", platform["name"], normalized_alias
            )
            ...
            if status in {200, 301, 302}:
                ...
                logger.warning(
                    "[SOCMINT_HIT] Public profile candidate on %s: %s",
                    platform["name"],
                    platform["url"],
                )
        except Exception as exc:
            logger.debug(
                "[SOCMINT] %s unavailable or not found: %s", platform["name"], exc
            )
